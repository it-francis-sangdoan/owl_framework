...
class Task extends Component {
    ...
    <div t-if="!state.isEditing" class="form-check form-switch fs-5">
            <input class="form-check-input" type="checkbox" value="" 
                t-att-id="state.id" 
                t-att-checked="state.isCompledted" 
                t-on-click="toggleTask"
                t-model="state.isCompledted"
            />
            <label class="form-check-label" 
                t-att-for="state.id" 
                t-attf-class="#{state.isCompledted ? 
    ...
    setup() {
        this.state = useState({
            ...
            isCompledted: this.props.task.isCompledted,
        })
    }

    toggleTask() {
        this.state.isCompledted = !this.state.isCompledted
    }
